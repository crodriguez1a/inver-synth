...
class DatasetCreator():
    ...
    def create_parameters(self,max: int = 10, method: str = 'complete', extra: dict = {}, force_create=False) -> str :
        filename = self.get_dataset_filename(  "data", 'hdf5')
        if os.path.isfile(filename) and not force_create:
            print("Parameter file exists, not recreating (use --regenerate_samples if you want to force)")
            return filename
        print("+"*40)
        ...
        with h5py.File(filename, 'w') as datafile:
            ...
            labels = datafile.create_dataset(
                "labels", (records, param_size))
    ...
    def generate_audio(self,  sound_generator: SoundGenerator, length: float = 0.1,
                 sample_rate: int = 44100, extra: dict = {},dataset_filename=None,force_generate=False):
        if dataset_filename is None:
            dataset_filename = self.get_dataset_filename(  "data", 'hdf5')

        print("+"*40)
        print(f"Generating Audio for Dataset {self.name} ({dataset_filename}), with {length}s at {sample_rate}/s")
        ...
        with h5py.File(dataset_filename, 'r+') as datafile:
            ...
            for index,filename in enumerate(filenames):
                if audio_exists[index] and os.path.isfile(filename) and not force_generate:
    ...
    def save_parameters(self):
        self.parameters.save_json(
            self.get_dataset_filename( "params", 'json'))
...
def generate_examples(gen:SoundGenerator,parameters:ParameterSet,args=None,extra={}):
    ...
    g.create_parameters(
                max=args.samples,
                method=args.method,
                force_create=args.regenerate_samples)
