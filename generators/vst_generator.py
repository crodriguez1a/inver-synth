...
def run_generator(name:str,plugin:str,config:str,max:int,
    ...
    g.generate(sound_generator=gen,length=length,sample_rate=sample_rate,method=method,max=max,extra={'note_length':note_length,'config':config})
    print("+"*50)
...
if __name__ == "__main__":
    ...
    print(args)
    if args.command == 'run':
        run_generator(args.name,args.plugin,args.config_file,args.samples,args.data_dir,args.wave_dir)
        pass
