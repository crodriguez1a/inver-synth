...
def raw_dataset(items: list, sample_rate: int = 16384, duration: float = 1.) -> np.ndarray:
    ...
    """
    t: tuple = tuple(utils.load_audio(
        item, sample_rate, duration)[0] for item in items)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-w', action="store", dest="wavs",
                        type=str, help="input wav files")
    parser.add_argument("-s", action="store", dest="save",
                        type=str, help="save numpy output")
